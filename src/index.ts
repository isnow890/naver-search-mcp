...
export const configSchema = z.object({
  naverClientId: z.string().describe("Naver API Client ID from Naver Developers Center"),
  naverClientSecret: z.string().describe("Naver API Client Secret from Naver Developers Center"),
});

// Smithery createServer function
export default function createServer({
  config,
}: {
  config: z.infer<typeof configSchema>;
}) {
  // Initialize Naver client with Smithery config
  ...
  // Register all tools
  const allTools = [...searchTools, ...datalabTools, ...utilityTools, ...categoryTools];
  for (let tool of allTools) {
    server.addTool(tool);
  }

  // Tool function wrapper (same as below)
